...
'''
    Para cada um dos processos jurídicos da pasta "Decisoes", este programa imprime, 
    numa tabela em formato xlsx:
   
    - o número do processo;
    - a jurisdição do processo;
    - a quantidade de arquivos existentes no processo;
    - a frequência com que no processo ocorrem palavras do grupo "sabidamente inverídico";
...
w = xlsxwriter.Workbook ("frequencias.xlsx")
...
for i in range (len (listaJurisdicoes)):
    ...
    for j in range (len (listaProcessos)):
        ...
        for k in range (len (listaArquivos)):
            ...
            ofens = re.findall (r"[Oo][Ff][Ee][Nn][Ss]\w*", texto)
